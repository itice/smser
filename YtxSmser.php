...
class YtxSmser extends Component {
	...
	private $Filename="../runtime/logs/log.txt"; //日志文件
	private $Handle;
    ...
    */       
    function sendTemplateSMS($to,$datas)
    {
        ...
        if($this->BodyType=="json"){
           ...
           $body= "{'to':'$to','templateId':'$this->TempId','appId':'$this->AppId','datas':[".$data."]}";
        }else{
           $data="";
           for($i=0;$i<count($datas);$i++){
           ...
           $body="<TemplateSMS>
                    <to>$to</to> 
                    <appId>$this->AppId</appId>
                    <templateId>$this->TempId</templateId>
                    <datas>".$data."</datas>
                  </TemplateSMS>";
        }
        $this->showlog("request body = ".$body);
